...
class MercadoLibre_Items_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
		try{
			$Message ="INFORMATION::Start Time for getMLCatergoriesAllDataAction ";
			...
			if(trim($resCheckUpdate['0']['update_id']) == '' || trim($resCheckUpdate['0']['update_id']) == '0'){
				...
				if (is_dir($dir)) {
					...
					$Message = "INFORMATION::All Categories already added in database";
					Mage::log($Message,null,"meli_backup_jobs.txt");
					Mage::log($e->getMessage(),null,"meli_backup_jobs.txt");
				}
		...
		$Message ="INFORMATION::End Time for getMLCatergoriesAllDataAction ";
		Mage::log($Message,null,"meli_backup_jobs.txt");	
    }
	...
	{
			try{
				$Message ="INFORMATION::Start Time for getMLCategoryHasAttributesAction ";
				Mage::log($Message,null,"meli_backup_jobs.txt");
				...
				try{
					if(count($dataMLcat) > 0){
						$dir = Mage::getBaseDir('var').DS.'atrrCategory';
						try{
							if (!is_dir($dir)) 
							{
								if (!@mkdir($dir , 0777, true)) 
							  ...
							  $Message = "Error::Unable to directory create name:atrrCategory";
							  Mage::log($Message,null,"meli_backup_jobs.txt");
							  Mage::log($e->getMessage(),null,"meli_backup_jobs.txt");
						}
						...
						try{
							if(count($attributesArr) > 0)
							{
								...
								*/
								...
								$fileName  ='';
								$fileName = strtolower($meli_category_id);
								if (!@file_put_contents($dir . DS . $fileName . '.txt',  $json_data)) {
									return false;
								}
							}
						}catch(Exception $e){
					...
					foreach($dataMLcat as $row){
						...
						if(count($attributesArr) > 0)
						{
							...
							foreach($attributesArr as $rowAttribute)
							{
								...
								$required  = (isset($rowAttribute['tags']->required))?$rowAttribute['tags']->required:'NULL';
